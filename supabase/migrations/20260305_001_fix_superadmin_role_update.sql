...
BEGIN
  ...
  -- Validate new role
  IF p_new_role NOT IN ('user', 'admin', 'superadmin', 'moderator', 'organizer_verified', 'organizer_pending', 'contributor') THEN
    RETURN jsonb_build_object('ok', false, 'error', 'invalid_role');
  END IF;

  -- Prevent admin/superadmin from demoting themselves (safety)
  IF v_caller_id = p_user_id AND p_new_role <> v_caller_role THEN
