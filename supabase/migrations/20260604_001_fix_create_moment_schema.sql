-- Fix create_moment function to use correct schema
-- The function was incorrectly querying events.moments_enabled and events.creator_id
-- but these columns are in event_settings and the column is created_by, not creator_id

-- Fix the create_moment function
...
BEGIN
  ...
  IF v_moments_who_can_post = 'rsvp' THEN
    IF v_user_role IS NULL AND v_event_creator_id != v_user_id THEN
      RAISE EXCEPTION 'not_allowed_to_post';
    END IF;
  ELSIF v_moments_who_can_post = 'confirmed' THEN
    IF v_user_role NOT IN ('going', 'host', 'cohost', 'speaker') AND v_event_creator_id != v_user_id THEN
      RAISE EXCEPTION 'not_allowed_to_post';
    END IF;
  END IF;
...
-- Fix the create_moments_batch function
