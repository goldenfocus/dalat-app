-- Fix create_moment and create_moments_batch functions
-- The 20260503 migration incorrectly assumed moments settings are on the events table,
-- but they're actually in the event_settings table.

-- ============================================
-- FIX create_moment function
-- ============================================
...
BEGIN
  ...
  -- Check if authenticated user can post (uses event_settings table internally)
  IF NOT public.can_post_moment(p_event_id) THEN
    RAISE EXCEPTION 'not_allowed_to_post';
  END IF;

  -- Check if approval is required (from event_settings table)
  ...
  -- Insert the moment with effective user ID, source locale, and thumbnail
  INSERT INTO moments (event_id, user_id, content_type, media_url, text_content, status, source_locale, thumbnail_url)
  VALUES (p_event_id, v_effective_uid, p_content_type, p_media_url, p_text_content, v_status, v_valid_locale, p_thumbnail_url)
...
-- ============================================
-- FIX create_moments_batch function
-- ============================================
...
BEGIN
  ...
  LOOP
    ...
    v_thumbnail_url := v_moment_record->>'thumbnail_url';
