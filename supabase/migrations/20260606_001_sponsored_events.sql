-- ============================================
-- SPONSORED/PREMIUM EVENTS
-- Migration: 20260606_001_sponsored_events
-- ============================================
-- Adds sponsor_tier to events for premium placement.
-- Sponsored events appear first in feeds.
--
-- Tier levels (expandable):
-- NULL = Not sponsored (default)
-- 1 = Basic sponsor
-- 2 = Premium sponsor
-- 3 = Gold sponsor (top tier)

-- Add sponsor_tier column to events
ALTER TABLE events
ADD COLUMN sponsor_tier smallint DEFAULT NULL;

-- Add index for efficient filtering/sorting
CREATE INDEX idx_events_sponsor_tier ON events(sponsor_tier)
WHERE sponsor_tier IS NOT NULL;

-- Add comment for documentation
COMMENT ON COLUMN events.sponsor_tier IS
'Premium sponsorship tier for promoted placement. NULL = not sponsored. Higher = more visibility.';

-- ============================================
-- UPDATE FEED FUNCTION TO SORT SPONSORED FIRST
-- ============================================
...
AS $$
  ...
  ORDER BY
    -- SPONSORED EVENTS FIRST (higher tier = higher priority)
    r.sponsor_tier DESC NULLS LAST,
    -- Then by date: Past = recent first, Others = soonest first
...
SPONSORED EVENTS (sponsor_tier > 0) appear first, sorted by tier.
