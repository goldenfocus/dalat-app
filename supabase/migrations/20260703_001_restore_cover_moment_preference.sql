-- ============================================
-- Fix: Restore cover_moment_id preference (AGAIN!)
--
-- The migration 20260626_001_moments_strip_audio_count.sql added audio counts
-- but accidentally removed the cover_moment_id preference that was restored in
-- 20260623_001_fix_moments_strip_cover.sql.
--
-- This is the THIRD time this has been fixed. Please be careful when modifying
-- this function to preserve ALL existing functionality:
-- 1. cover_moment_id preference (manually selected covers)
-- 2. User attendance preference
-- 3. Quality score preference
-- 4. Photo/video/audio counts
-- ============================================
...
AS $$
  ...
  SELECT * FROM (
    SELECT DISTINCT ON (m.event_id)
      ...
      -- Count photos, videos, and audio for this event
