...
export async function CommunityNextEvent({ event, locale, communitySlug, rsvpStatus }: { event: { slug: string; title: string; image_url: string | null; starts_at: string; location_name: string | null }; locale: string; communitySlug: string; rsvpStatus?: string | null }) {
  const t = await getTranslations('tribes');
  const rsvp = await getTranslations('rsvp');
  const feed = await getTranslations('feed');
  ...
  return <section aria-label={t('nextGathering')} className="rounded-2xl border bg-card overflow-hidden shadow-sm">
    <Link href={`/events/${event.slug}?fromCommunity=${encodeURIComponent(communitySlug)}`} className={event.image_url ? "grid sm:grid-cols-[2fr_3fr] group" : "block group"}>
      ...
      <div className="min-w-0 p-5 sm:p-8 space-y-4">
        ...
        {rsvpStatus === 'going' && <p className="font-semibold text-primary">{rsvp('youreGoing')}</p>}
        {rsvpStatus === 'interested' && <p className="font-semibold text-blue-600 dark:text-blue-400">{rsvp('youreInterested')}</p>}
        <span className="inline-flex max-w-full items-center gap-2 rounded-lg bg-primary px-4 py-3 text-primary-foreground font-medium">{rsvpStatus && rsvpStatus !== 'cancelled' ? feed('viewEvent') : t('viewEventRsvp')}<ArrowUpRight className="h-4 w-4" /></span>
      </div>
    </Link>
  </section>;
}
