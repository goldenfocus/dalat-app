...
Long recaps summarize every evidence chunk before final writing, so historical
events with large transcripts cannot overflow the local model context. Invalid
model output uses the retry budget instead of blocking the queue indefinitely.
