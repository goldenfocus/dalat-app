...
import { enqueueCaptionJob, isCaptionWorkerAlive } from "@/lib/ai/caption-jobs";
 ...
 * Audio: local full-track transcription through the same caption worker.
...
export async function GET(request: Request) {
  ...
  for (
    let offset = 0;
    offset < MAX_SCAN && pending.length < limit * 2;
    offset += PAGE_SIZE
  ) {
    ...
    for (let i = 0; i < pageIds.length; i += IN_CHUNK) {
      ...
      const { data: settledRows, error: settledError } = await supabase
        ...
        .select(
          "moment_id, processing_status, video_transcript, audio_transcript",
        )
        .in("moment_id", chunk)
      ...
      const kinds = new Map(page.map((m) => [m.id, m.content_type]));
      for (const row of settledRows || []) {
        const kind = kinds.get(row.moment_id);
        if (
          row.processing_status === "completed" &&
          ((kind === "video" && row.video_transcript == null) ||
            (kind === "audio" && row.audio_transcript == null))
        )
          continue;
        settledIds.add(row.moment_id);
      }
    }
  ...
  const settleSkipped = async (momentId: string, reason: string) => {
    ...
    if (error) {
      console.error(
        `[process-moments] skip upsert failed for ${momentId}:`,
        error,
      );
      failed++;
  ...
  for (const moment of batch) {
    ...
    try {
      switch (moment.content_type) {
        ...
        case "audio": {
          ...
          if (
            ...
            if (await settleSkipped(moment.id, "never_transcoded"))
              skippedUnsupported++;
            break;
          }
        ...
        case "document": {
          ...
          if (await settleSkipped(moment.id, "analyzer_unavailable"))
            skippedUnsupported++;
          break;
        }
      ...
      const { error: failUpsertError } = await supabase.rpc(
        "upsert_moment_metadata",
        {
      ...
      if (failUpsertError) {
        console.error(
          `[process-moments] failed-status upsert also failed for ${moment.id}:`,
          failUpsertError,
        );
      }
    }
