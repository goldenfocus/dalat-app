...
export default async function CommunityInsights({params}:{params:Promise<{slug:string}>}) {
 ...
 return <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
  ...
  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{cards.map(([key,value,Icon])=><StatCard wrapTitle key={key} title={t(key)} value={value} icon={<Icon className="h-5 w-5"/>}/>)}</div>
